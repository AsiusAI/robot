  ...
  def run_sim_loop(self):
    ...
    while self.running:
      ...
      for arm_queue in [self.left_arm_queue, self.right_arm_queue]:
        ...
        # debug line
        end_pos = np.array(pos.position) + 4 * np.array(R.from_quat(pos.orientation).apply([0, 0, 1]))
        line = p.addUserDebugLine(pos.position, end_pos.tolist(), [1, 0, 0], lineWidth=2, lifeTime=0)
