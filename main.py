...
def init_yolo_model():
    ...
    global yolo_model, co_helper
    try:
        model_path = "yolov8.rknn"
        if os.path.exists(model_path):
            print("Initializing YOLO model...")
            yolo_model = RKNN_model_container(model_path, target='rk3588')
...
def draw_detections(image, boxes, scores, classes):
    ...
    for box, score, cl in zip(boxes, scores, classes):
        ...
        cv2.rectangle(image, (top, left - label_size[1] - 10), (top + label_size[0], left), (0, 255, 0), -1)
        cv2.putText(image, label, (top, left - 5), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 0, 0), 2)
    
    return image

def process_frame_for_yolo(frame):
    """Process frame for YOLO detection"""
    if yolo_model is None or co_helper is None:
        return frame
    ...
    try:
        ...
        # Run inference
        outputs = yolo_model.run([img])
        boxes, classes, scores = post_process(outputs)
        
        # Draw detections
        if boxes is not None:
            real_boxes = co_helper.get_real_box(boxes)
            frame = draw_detections(frame, real_boxes, scores, classes)
...
def cleanup_yolo():
    """Cleanup function to release YOLO model"""
    global yolo_model
    if yolo_model:
        ...
        try:
            ...
            print(f"Error during YOLO cleanup: {e}")
...
def generate_frames():
    while True:
        ...
        if not success:
            ...
            frame = process_frame_for_yolo(frame)
